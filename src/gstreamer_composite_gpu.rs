// GPU-ACCELERATED GStreamer composite pipeline for OBS-like functionality
// Uses glvideomixer and glalpha for real-time chroma key with zero CPU copies
...
// ============================================================================
// Main Composite Pipeline with GPU Acceleration
// ============================================================================
...
impl GStreamerComposite {
    ...
    pub fn play_fx_from_file(&mut self, file_path: String, _keycolor: String, _tolerance: f64, _similarity: f64, use_chroma_key: bool) -> Result<(), String> {
    ...
    pub fn emergency_cleanup(&self) -> Result<(), String> {
        ...
        if let Some(pipeline) = &self.pipeline {
            ...
            if let Some(found_bin) = pipeline.by_name("fxbin") {
                ...
                if has_current_fx {
                    println!("[Composite FX] ✅ Current FX is active - skipping emergency cleanup");
                    return Ok(());
                }
