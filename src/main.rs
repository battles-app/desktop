...
lazy_static::lazy_static! {
    static ref GSTREAMER_CAMERA: Arc<parking_lot::RwLock<Option<GStreamerCamera>>> = 
        Arc::new(parking_lot::RwLock::new(None));
    static ref CAMERA_FRAME_SENDER: Arc<parking_lot::RwLock<Option<broadcast::Sender<Vec<u8>>>>> = 
        Arc::new(parking_lot::RwLock::new(None));
    
    static ref GSTREAMER_COMPOSITE: Arc<parking_lot::RwLock<Option<GStreamerComposite>>> = 
        Arc::new(parking_lot::RwLock::new(None));
    static ref COMPOSITE_FRAME_SENDER: Arc<parking_lot::RwLock<Option<broadcast::Sender<Vec<u8>>>>> = 
        Arc::new(parking_lot::RwLock::new(None));
    static ref CAMERA_LAYER_FRAME_SENDER: Arc<parking_lot::RwLock<Option<broadcast::Sender<Vec<u8>>>>> = 
        Arc::new(parking_lot::RwLock::new(None));
    static ref OVERLAY_LAYER_FRAME_SENDER: Arc<parking_lot::RwLock<Option<broadcast::Sender<Vec<u8>>>>> = 
        Arc::new(parking_lot::RwLock::new(None));
}
...
// Fallback function using Windows GDI
fn capture_monitor_screenshot_fallback(x: i32, y: i32, width: i32, height: i32) -> Option<String> {
    #[cfg(target_os = "windows")]
    {
        ...
        unsafe {
            ...
            if result == 0 || buffer.iter().all(|&b| b == 0) {
                return None;
            }
            ...
            let base64_string = base64::Engine::encode(&base64::engine::general_purpose::STANDARD, &png_buffer);
...
// Read monitors using Tauri's built-in API
...
async fn create_monitor_window(
    ...
    if monitor_index >= native_monitors.len() {
        return Err(format!("Invalid monitor index: {}", monitor_index));
    }
...
async fn play_composite_fx(
    ...
    if !local_path.exists() {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let response = client.get(&full_url_clone).send()
